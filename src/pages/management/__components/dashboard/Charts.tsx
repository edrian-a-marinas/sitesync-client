...
export function BudgetVsActualChart({
  ...
  return (
    <ChartCard
      title="Budget vs Actual (Active)"
      subtitle="Per project, in ₱ millions"
    >
...
export function MaterialConsumptionChart({
  ...
  data.forEach((d) => {
    totalCostByMaterial.set(
      d.material_name,
      (totalCostByMaterial.get(d.material_name) ?? 0) + d.total_cost,
    )
  })
  const materialNames = Array.from(totalCostByMaterial.keys()).sort(
    (a, b) =>
      (totalCostByMaterial.get(b) ?? 0) - (totalCostByMaterial.get(a) ?? 0),
  )
  ...
  const chartData = weeks.map((week) => {
    ...
    materialNames.forEach((name) => {
      const entry = data.find(
        (d) => d.week === week && d.material_name === name,
      )
  ...
  return (
    <ChartCard
      ...
      subtitle="Last 8 weeks, costs by material"
    >
      <ResponsiveContainer width="100%" height="100%">
        <LineChart
          data={chartData}
