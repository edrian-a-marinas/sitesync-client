...
export function OwnerFilterBar({
  ...
  const handleYearChange = (v: string) => {
    ...
    onFiltersChange({
      year,
      projectId: year !== 'all' ? 'all' : filters.projectId,
    })
  }
  ...
  const handleProjectChange = (v: string) => {
    ...
    onFiltersChange({
      projectId,
      year: projectId !== 'all' ? 'all' : filters.year,
    })
  }
    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      >
        ...
        >
          <SelectValue
            placeholder={
              isLoading ? 'Loading...' : isError ? 'Failed' : 'All Projects'
            }
          />
        </SelectTrigger>
...
export function ProjectScopeToggle({
  ...
  return (
    <Select
      ...
      onValueChange={(v) =>
        onSelectionChange(v === 'aggregate' ? 'aggregate' : Number(v))
      }
      disabled={isLoading || isError}
    >
