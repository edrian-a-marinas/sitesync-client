    ...
    <>
      ...
      {kpi.delta != null && (
        <Delta
          ...
          inverted={kpi.deltaInvertedColor}
        />
      )}
    </>
  )
...
export function OwnerKPICards({
  ...
  data: OwnerDashboard
  filteredProjects?: ProjectBudgetSummary[]
  year?: number | 'all'
}) {
  ...
  const kpis: KPI[] = [
    ...
    },
    {
      ...
      icon: Users,
      delta:
        showDeltas && !isSingleProject ? data.total_workers_active_delta : null,
      deltaLabel: 'vs last week',
    },
    {
      label:
        isSingleProject || isPastYearFiltered
          ? 'Total Incidents'
          : 'Incidents This Week',
      value: String(totalIncidents),
      icon: AlertTriangle,
      delta:
        showDeltas && !isSingleProject ? data.incidents_this_week_delta : null,
      deltaInvertedColor: true,
    },
  ]
  return (
...
export function ManagerKPICards({ data }: { data: ProjectManagerDashboard }) {
  const budgetPercent =
    data.phases.reduce((sum, p) => sum + p.allocated_budget, 0) > 0
      ? Math.round(
          (data.total_material_cost /
            data.phases.reduce((sum, p) => sum + p.allocated_budget, 0)) *
            100,
        )
      : 0
  const totalBudget = data.phases.reduce(
  ...
  const kpis: KPI[] = [
    ...
    {
      label:
        data.status === 'Completed' ? 'Total Incidents' : 'Incidents This Week',
      value:
        data.status === 'Completed'
          ? String(data.total_incidents)
          : String(data.incidents_this_week),
      icon: AlertTriangle,
      delta:
        data.status === 'Completed' ? null : data.incidents_this_week_delta,
      deltaLabel: 'vs last week',
      deltaInvertedColor: true,
    },
  ]
...
export function ManagerAggregateKPICards({
  data,
}: {
  data: ProjectManagerAggregateDashboard
}) {
  const budgetPercent =
    data.total_budget > 0
