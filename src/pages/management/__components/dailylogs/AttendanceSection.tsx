...
export default function AttendanceSection({
  ...
  const availableWorkers = (project?.workers ?? []).filter(
    (w) => !loggedWorkerIds.has(w.id),
  )
      ...
      onError: (err: unknown) => {
        const message =
          (err as { response?: { data?: { detail?: string } } })?.response?.data
            ?.detail ?? 'Failed to log attendance. Please try again.'
        toast.error(message)
      },
    })
  }
      ...
      <div className="flex items-center justify-between">
        <span className="text-xs text-zinc-400 dark:text-zinc-500">
          {attendance && attendance.length > 0
            ? `${attendance.length} worker${attendance.length !== 1 ? 's' : ''}`
            : ''}
        </span>
        {canEdit && !showForm && (
          <TooltipProvider delayDuration={200}>
            <Tooltip>
              <TooltipTrigger asChild>
                <span
                  className={
                    availableWorkers.length === 0 ? 'cursor-not-allowed' : ''
                  }
                >
                  <Button
                    variant="outline"
