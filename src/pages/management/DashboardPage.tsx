...
import {
  useOwnerDashboard,
  useManagerDashboard,
  useManagerAggregateDashboard,
} from '@/hooks/useDashboard'
import {
  OwnerKPICards,
  ManagerKPICards,
  ManagerAggregateKPICards,
} from './__components/dashboard/KPICards'
import {
  BudgetVsActualChart,
  MaterialConsumptionChart,
} from './__components/dashboard/Charts'
...
export default function DashboardPage() {
  ...
  const [scopeSelection, setScopeSelection] =
    useState<ScopeSelection>('aggregate')
  const [ownerFilters, setOwnerFilters] =
    useState<DashboardFilters>(DEFAULT_FILTERS)

  const ownerYear =
    isOwner && ownerFilters.year !== 'all' ? ownerFilters.year : undefined
  const {
    data: ownerData,
    isLoading: ownerLoading,
    isError: ownerError,
  } = useOwnerDashboard(isOwner, ownerYear)
  const {
    data: managerData,
  ...
  } = useManagerAggregateDashboard(!isOwner && scopeSelection === 'aggregate')
  ...
  const ownerMaterialScope: ScopeSelection = isOwnerCurrentYear
    ? 'aggregate'
    : 1
  ...
  const renderOwnerKPIs = () => {
    ...
    return (
      <OwnerKPICards
        data={ownerData}
        filteredProjects={ownerChartData?.projects}
        year={ownerFilters.year}
      />
    )
  }

  const renderManagerKPIs = () => {
    const isManagerLoading =
      scopeSelection === 'aggregate' ? aggregateLoading : managerLoading
    const isManagerError =
      scopeSelection === 'aggregate' ? aggregateError : managerError
  ...
  const activeData =
    scopeSelection === 'aggregate' ? aggregateData : managerData
                            ...
                            ? [
                                {
                                  ...
                                  status: 'Active',
                                  total_budget: managerData.phases.reduce(
                                    (s, p) => s + p.allocated_budget,
                                    0,
                                  ),
                                  actual_spending:
                                    managerData.total_material_cost,
                                  is_over_budget:
                                    managerData.total_material_cost >
                                    managerData.phases.reduce(
                                      (s, p) => s + p.allocated_budget,
                                      0,
                                    ),
