...
export default function OwnerAiAssistantPage() {
  ...
  const { mutate: deleteAllQueries, isPending: isDeletingAll } =
    useDeleteAllQueries()
  ...
  const handleProjectChange = (projectId: number | null) => {
    ...
    setScopeMarkers((prev) => [
      ...prev,
      { id: `scope-${Date.now()}`, timestamp: Date.now(), projectName },
    ])
  }
  ...
  const handleDeleteAll = () => {
    deleteAllQueries(undefined, {
      onSuccess: (data) =>
        toast.success(
          `Deleted ${data.deleted} ${data.deleted === 1 ? 'query' : 'queries'}.`,
        ),
      onError: () =>
  ...
  const handleSubmit = (text?: string) => {
    ...
    if (!trimmed || isSubmitting || isRateLimited || isWaitingForResponse)
      return
    createQuery(
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 pb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
            Ask questions about your projects, budgets, materials, and
            workforce.
          </p>
        </div>
      </div>
      ...
      {isRateLimited && (
        <Alert className="mb-4 border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-900/20">
          ...
          <AlertDescription className="text-amber-700 dark:text-amber-400">
            Groq API rate limit reached. Chat available again in{' '}
            <strong>{formatCooldown(cooldownLeft)}</strong>.
          </AlertDescription>
        </Alert>
      )}
