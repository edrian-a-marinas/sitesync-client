...
export const getDailyLogs = async (projectId: number, page: number, pageSize: number, search?: string): Promise<DailyLogListResponse> => {
  const response = await getDailyLogsRequest(projectId, page, pageSize, search)
