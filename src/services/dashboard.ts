...
export const getManagerDashboard = async (projectId: number): Promise<ProjectManagerDashboard> => {
  const response = await getManagerDashboardRequest(projectId);
  return response.data;
};

export const getManagerAggregateDashboard = async (): Promise<ProjectManagerAggregateDashboard> => {
