...
export const getEquipment = async (projectId: number, logId: number): Promise<EquipmentResponse[]> => {
  const response = await getEquipmentRequest(projectId, logId)
  return response.data
}

export const createEquipment = async (projectId: number, logId: number, data: EquipmentCreate): Promise<EquipmentResponse> => {
